use crate::{
  ...
  packet::{compose_icmp, parse_icmp, parse_tcp, parse_udp, IpOverMac},
  ASockProtocol,
};
use pnet::packet::{
...
impl IpLayerInternal {
  ...
  fn handle_ping(&mut self, icmp: Icmp, from: Ipv4Addr) {
    if icmp.icmp_type == IcmpTypes::EchoRequest {
      log::debug!(
        "response to ping request from {:?}, with payload {:?}",
        from,
        icmp.payload
      );
      let icmp = Icmp {
      ...
      let ipv4 = compose_icmp(&icmp, self.self_ip, from);
  ...
  fn on_recv_ipv4(&mut self, ipv4: Ipv4) {
    ...
    if ipv4.destination != self.self_ip {
      log::debug!("not targeting us, discard the pacet");
      return;
    }

    // possible a ping request
